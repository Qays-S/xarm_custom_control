#!/usr/bin/env python3
"""
Move the xArm7 through a full pick-and-place sequence, for REAL HARDWARE
(gripper via GripperCommand, not the fake sim's FollowJointTrajectory).

Run under: ros2 launch xarm_planner xarm7_planner_realmove.launch.py
           robot_ip:=192.168.1.210
           (add_gripper is already forwarded to true by the source patch)
...
SEQUENCE = [
    ("gripper", GRIPPER_OPEN_REAL),
    ("arm", [0.0, -1.4661, 0.0, 0.1745, 0.0, 1.6580, 0.0]),                # stop 1
   # ("arm", [-0.7767, -1.4661, 0.0, 0.1745, 0.0, 1.6580, 0.0]),            # stop 1.5 - halfway
    #("arm", [-1.5534, -1.4661, 0.0, 0.1745, 0.0, 1.6580, 0.0]),            # stop 2
    #("arm", [-1.5534, -0.5585, 0.0, 0.1745, 0.0, 0.7330, 0.0]),            # stop 3
    #("gripper", GRIPPER_GRAB_BOX_REAL),
    #("arm", [-1.5534, -0.7854, 0.0, 0.7330, 0.0, 1.5184, 0.0]),            # stop 4 / clearance
    #("arm", [-0.4363, -0.7854, 0.0, 0.7330, 0.0, 1.5184, 0.0]),            # next
    #("arm", [-0.4363, 0.3840, 0.0, 1.9722, 0.0, 1.5882, 0.0]),             # next (arm)
    #("arm", [-0.4363, 0.4712, 0.0, 1.4137, 0.0, 0.9425, 0.0]),             # new arm move
    #("gripper", GRIPPER_OPEN_REAL),                                        # release object
    #("arm", [-0.4363, 0.3840, 0.0, 1.9722, 0.0, 1.5882, 0.0]),             # pre-home
...
def main():
    rclpy.init()
    node = Node("xarm_pick_and_place_sequence")
